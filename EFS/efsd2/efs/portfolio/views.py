...
def mutual_new(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            mutual.save()
            mutuals = Mutual.objects.filter(acquired_date__lte=timezone.now())
...
def register(response):
    ...
    return render(response, "portfolio/register.html", {"form": form})
...
def customer_portfolio(request):
        ...
        queryset = Investment.objects.order_by('-recent_value')[:5]
        for investment in queryset:
            labels.append(investment.category)
            data.append(investment.recent_value)

        return render(request, 'portfolio/customer_portfolio.html', {
            'labels': labels,
            'data': data,
        })
